...
# Update the app.run section to bind to 0.0.0.0 and use the port provided by Render
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 5000))  # Get the port from environment or default to 5000
    app.run(host="0.0.0.0", port=port, debug=True)  # Bind to 0.0.0.0 to listen on all interfaces
